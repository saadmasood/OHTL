...
function QueriesListView({route, navigation}) {
  const data3 = {
    DAssignedDC: '0',
    DDCPerformed: '0',
    DPendingDC: '0',
  ...
  const loadData = val => {
    let data1;
    AsyncStorage.getItem('QueriesData').then(items => {
      var data = [];
  ...
  useEffect(() => {
    ...
    if (route.params.otherParam == 'NewCases') {
      setCaseTypeHeading('Queries List');
            ...
            return (
              <Swipeable>
                <TouchableOpacity
                  // disabled={true}
                  onPress={() => {
                    navigation.navigate('QueriesScreen', {
                      data: item,
                      index: index,
                      otherParam: item.Objnr,
...
export default QueriesListView;
